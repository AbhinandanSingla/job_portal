...
export default function RoundDiv({
  ...
  useEffect(() => {
    const watcherId = attachCSSWatcher(updateStatesWithArgs, div.current);
  ...
  const svgRef = useRef();
  useEffect(() => {
    setInnerPath(generateSvgSquircle(height - (border.width[0] + border.width[2]), width - (border.width[1] + border.width[3]), radius.map((val, i) => Math.max(0, val - Math.max(border.width[i], border.width[i === 0 ? 3 : i - 1])))).replace(/(\d+(\.\d+)?),(\d+(\.\d+)?)/g, match => match.split(',').map((number, i) => Number(number) + (i === 0 ? border.width[3] : border.width[0])).join(',')));
    setPath(generateSvgSquircle(height, width, radius));
  }, [height, width, radius, border, padding]);
  useEffect(() => {
    const currentSvgRef = svgRef.current;
    if (currentSvgRef) setTimeout(() => {
  ...
  const divStyle = { ...style,
    borderImage: 'none',
    ...(pathIsEmpty ? {} : { ...invisibleBorderStyles,
      padding: '0',
      background: 'transparent',
      boxShadow: dontConvertShadow ? shadows[0].length === 0 ? 'none' : shadows[0].join(',') : 'none',
  ...
  if (div.current) div.current.rrdOverwritten = !pathIsEmpty;
  return React.createElement("div", _extends({}, props, {
    style: divStyle,
    ref: div
  }), pathIsEmpty ? null : React.createElement(ShadowRoot, null, React.createElement("div", {
    style: {
      transform: 'scale(1)'
    }
  }, React.createElement("div", {
    style: { ...shapeComponentStyles,
    ...
    style: {
      ...
      ...Object.fromEntries(Object.keys(borderImage).map(key => {
        return [camelise('border-image-' + key), borderImage[key] + (key === 'slice' ? ' fill' : '')];
      })),
      transition
    }
  })), React.createElement("div", {
    ...
    ref: slotWrapper
  }, React.createElement("slot", null)))), children);
}